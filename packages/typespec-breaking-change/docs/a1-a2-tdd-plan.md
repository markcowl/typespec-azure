...
Features A1 and A2 close the biggest remaining suppression gaps in the breaking-change tool:

- **A1: New vs Existing Suppression Classification** tells CI whether an approval already existed on the base branch or was introduced/changed in the PR.
- **A2: Version Scoping with `since:`** makes approvals precise when the same breaking change shape appears in more than one stable version transition.
...
The **identity key** for matching is: `(decorator, anchorIdentity, placement, kind, path)`.
Two suppressions on the same declaration with different `kind` values are independent and must be matched/classified separately.

The **metadata** for modification detection is: `(since, reason)`.
...
- **Identity key**: decorator family + anchor identity + placement + DiffKind + normalized path
- **Metadata key**: since + reason

The `DiffKind` (the `kind` option) is part of the identity key, NOT metadata. Two suppressions on the same declaration with different DiffKinds (e.g., `ResourcePropertyTypeChanged` vs `ResourcePropertyRemoved`) are independent suppressions and must be matched independently.

Matching rules:

- **Direct placement**: compare by declaration identity + DiffKind
- **Parent placement**: compare by ancestor identity + DiffKind + `path`

Do **not** include `reason` or `since` in the identity key; those are metadata used to detect modifications.

**Tests to write first**

1. Direct suppression on the same property with same DiffKind matches across programs.
2. Direct suppression on the same property with different DiffKind does NOT match (treated as independent).
3. Parent suppression on the same model + same `path` + same DiffKind matches across programs.
4. Parent suppression on the same model + same `path` + different DiffKind does NOT match.
5. Parent suppression on the same model + different `path` does not match.
6. Operation-level suppression matches by normalized operation identity, not source position.
7. Two decorators on same node with different DiffKinds are collected and matched independently.
...
```typespec
@approvedBreakingChange("approved", #{ kind: "ResourcePropertyRemoved", path: "properties.legacy" })
model Widget {
...
Expected: EXISTING (same identity key: decorator=approvedBreakingChange, anchor=Widget, kind=ResourcePropertyRemoved, path=properties.legacy)

**Counter-example: different DiffKind = independent**

Base:

```typespec
model Widget {
  @approvedBreakingChange("type changed", #{ kind: "ResourcePropertyTypeChanged" })
...
model Widget {
  @approvedBreakingChange("type changed", #{ kind: "ResourcePropertyTypeChanged" })
  @approvedBreakingChange("made optional", #{ kind: "ResourcePropertyMadeOptional" })
  @typeChangedFrom(Versions.v2, int32)
  age?: int64;
}
```

Expected: `ResourcePropertyTypeChanged` suppression → EXISTING; `ResourcePropertyMadeOptional` suppression → NEW
...
```typespec
model Widget {
  @approvedBreakingChange("old reason", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
```typespec
model Widget {
  @approvedBreakingChange("new reason", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```

Expected: `MODIFIED` (same identity key, but `reason` metadata differs)
...
```typespec
model Widget {
  @approvedBreakingChange("approved in this PR", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
model Widget {
  @approvedBreakingChange("legacyStatus removal approved", #{ kind: "ResourcePropertyRemoved" })
...
```typespec
model Widget {
  @approvedBreakingChange("approved in PR", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
```typespec
model Widget {
  @approvedBreakingChange("legacy removal approved", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
```typespec
@approvedBreakingChange("reason v2", #{ kind: "ResourcePropertyRemoved", path: "properties.legacy" })
model Widget {
  @removed(Versions.v2)
  legacy?: string;
}
...
```typespec
model Widget {
  @approvedBreakingChange("direct", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
```typespec
@approvedBreakingChange("parent", #{ kind: "ResourcePropertyRemoved", path: "properties.legacy" })
model Widget {
  @removed(Versions.v2)
  legacy?: string;
}
...
```typespec
model Widget {
  @approvedBreakingChange("single occurrence approved", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2)
  legacy?: string;
}
```
...
```typespec
model Widget {
  @approvedBreakingChange("too broad", #{ kind: "ResourcePropertyRemoved" })
  @removed(Versions.v2024)
  @added(Versions.v2025)
  @removed(Versions.v2026)
  legacy?: string;
...
```typespec
model Widget {
  @approvedBreakingChange("first removal", #{ kind: "ResourcePropertyRemoved", since: "2024-01-01" })
  @approvedBreakingChange("second removal", #{ kind: "ResourcePropertyRemoved", since: "2026-01-01" })
  @removed(Versions.v2024)
  @added(Versions.v2025)
  @removed(Versions.v2026)
  legacyStatus?: string;
...
```typespec
model Widget {
  @approvedBreakingChange("direct scoped", #{ kind: "ResourcePropertyRemoved", since: "2024-01-01" })
  @removed(Versions.v2024)
  legacy?: string;
}
```
...
- direct placement uses declaration identity + DiffKind + exact head version
